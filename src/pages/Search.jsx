...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios(
        `http://fear.master.com:4000/fear/api/products?queryString=${e.target.search.value}&&_page=${currentPage}`
      );
  ...
  const handleSearchPagination = async () => {
    try {
        const response = await axios(
          `http://fear.master.com:4000/fear/api/products?queryString=${searchTerm}&&page=${currentPage}`
        );
        const data = response.data;
        setProducts(data);
      } catch (error) {
